...
static void inline CreateMatrix(float* host_data, float* host_a,
		float* host_b) {
	int shift = 0;
	for (int i = 0; i < 6; ++i)
		for (int j = i; j < 7; ++j) {
...
struct RGBReduce {
	...
	__device__ inline
	bool SearchPoint(int& x, int& y, float& gx, float& gy, float& diff,
			float3& vlast_c) const {
...
__global__ void RGBReduceSum_device(const RGBReduce rgb) {
...
struct RGB {
	...
	__device__ bool FindCorresp(int& x, int& y, int& u, int& v, float& gx,
...
__global__ void RGBDense_device(const RGB rgb) {

...
struct Projection
{
	...
	__device__ __inline__ bool projectBlock(const int3 & pos, RenderingBlock & block) const
		...
		for (int corner = 0; corner < 8; ++corner)
		{
			...
			float2 pt2d = project(pt3d) / DEPTH_MAP_SUBSAMPLE;
		...
		for (int y = 0; y < ny; ++y)
			for (int x = 0; x < ny; ++x)
			{
				if (offset < renderingBlockList.size)
				{
					...
					b.upperLeft.x = block.upperLeft.x + x * RENDERING_BLOCK_SIZE_X;
					b.upperLeft.y = block.upperLeft.y + y * RENDERING_BLOCK_SIZE_Y;
					b.lowerRight.x = block.upperLeft.x + RENDERING_BLOCK_SIZE_X;
					b.lowerRight.y = block.upperLeft.y + RENDERING_BLOCK_SIZE_Y;
	...
	__device__ __inline__ void operator()() const
	{
		...
		if (valid && offset != -1 &&
						(offset + requiredNoBlocks) < param.maxNumRenderingBlocks)
				...
				float cx, float cy)
{
	...
	dim3 block, thread;
	thread = dim3(16, 4);
	block.x = div_up(cols, thread.x);
	block.y = div_up(rows, thread.y);
	...
	thread = dim3(1024);
	block = dim3(div_up((int) noVisibleBlocks, block.x));
...
//		return voxel.sdf;
		Voxel *voxel = nullptr;
		map.find_voxel(make_int3(pt3d), voxel);
		if (voxel != nullptr)
		{
			valid = true;
			return voxel->sdf_;
...
//		return voxel.color;
	...
	__device__ __inline__ float readSdfInterped(const float3 & pt, HashEntry & cache, bool & valid)
	{
		...
		sdf[0] = readSdf(pt, cache, valid);
		sdf[1] = readSdf(pt + make_float3(1, 0, 0), cache, valid);
		result[0] = (1.0f - xyz.x) * sdf[0] + xyz.x * sdf[1];

		sdf[0] = readSdf(pt + make_float3(0, 1, 0), cache, valid);
		sdf[1] = readSdf(pt + make_float3(1, 1, 0), cache, valid);
		result[1] = (1.0f - xyz.x) * sdf[0] + xyz.x * sdf[1];
		result[2] = (1.0f - xyz.y) * result[0] + xyz.y * result[1];

		sdf[0] = readSdf(pt + make_float3(0, 0, 1), cache, valid);
		sdf[1] = readSdf(pt + make_float3(1, 0, 1), cache, valid);
		result[0] = (1.0f - xyz.x) * sdf[0] + xyz.x * sdf[1];

		sdf[0] = readSdf(pt + make_float3(0, 1, 1), cache, valid);
		sdf[1] = readSdf(pt + make_float3(1, 1, 1), cache, valid);
		result[1] = (1.0f - xyz.x) * sdf[0] + xyz.x * sdf[1];
	...
	__device__ __inline__ void operator()()
	{
		...
		int2 locId;
		locId.x = __float2int_rd((float) x / DEPTH_MAP_SUBSAMPLE);
		locId.y = __float2int_rd((float) y / DEPTH_MAP_SUBSAMPLE);
		...
		float dist_s = norm(pt3d) * param.inverse_voxel_size();
		float3 block_s = (Rview * pt3d + tview) * param.inverse_voxel_size();

		pt3d.z = zRange.y;
		pt3d.x = pt3d.z * ((float) x - cx) * invfx;
		pt3d.y = pt3d.z * ((float) y - cy) * invfy;
		float dist_e = norm(pt3d) * param.inverse_voxel_size();
		float3 block_e = (Rview * pt3d + tview) * param.inverse_voxel_size();
		...
		if (found_pt)
		{
			...
			{
				result = RviewInv * (result * param.voxel_size_ - tview);

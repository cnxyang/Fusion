...
bool CreateRenderingBlocks(MapStruct* map, DeviceArray2D<float> & zRangeX,
		DeviceArray2D<float> & zRangeY, const float & depthMax,
...
uint MeshScene(DeviceArray<uint> & noOccupiedBlocks,
		DeviceArray<uint> & noTotalTriangles, MapStruct map,
...
void CheckBlockVisibility(MapStruct map, DeviceArray<uint> & noVisibleBlocks,
		...
		float fx, float fy, float cx, float cy, uint * host_data);
...
void RGBStep(const DeviceArray2D<unsigned char>& nextImage,
		...
		const DeviceArray2D<float4>& lastVMap, const DeviceArray2D<short>& dIdx,
		const DeviceArray2D<short>& dIdy, Matrix3f Rcurr, Matrix3f RcurrInv,
		Matrix3f Rlast, Matrix3f RlastInv, float3 tcurr, float3 tlast, float* K,
		DeviceArray2D<float> & sum, DeviceArray<float> & out,
		DeviceArray2D<int> & sumRes, DeviceArray<int> & outRes,
...
void FuseKeyFrameDepth(DeviceArray2D<float>& lastDMap,
		DeviceArray2D<float>& nextDMap, DeviceArray2D<int>& lastWMap,
		DeviceArray2D<float4>& nextNMap, DeviceArray2D<float4>& nextVMap,
		Matrix3f R, float3 t, float* K);
...
void CheckVisibility(DeviceArray<float3> & pt3d, DeviceArray<float2> & pt2d,
		DeviceArray<int> & match, Matrix3f RcurrInv, float3 tcurr,

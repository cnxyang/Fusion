...
struct MeshEngine
{
	...
	__device__ inline void checkBlocks()
	{
		...
		uint val = 0;
		if (x < param.maxNumHashEntries && map.hashEntries[x].next >= 0)
		{
			int3 pos = map.hashEntries[x].pos * BLOCK_SIZE;
			scan = true;
			val = 1;
		}
	...
	__device__ inline bool readNormal(float3* n, float* sdf, int3 pos)
	{
		...
		float v1, v2, v3;
		v1 = map.FindVoxel(pos + make_int3(-1, 0, 0)).sdf_;
		v2 = map.FindVoxel(pos + make_int3(0, -1, 0)).sdf_;
		v3 = map.FindVoxel(pos + make_int3(0, 0, -1)).sdf_;
		if (isnan(v1) || isnan(v2) || isnan(v3))
			return false;
		n[0] = make_float3(sdf[1] - v1, sdf[3] - v2, sdf[4] - v3);

		v1 = map.FindVoxel(pos + make_int3(2, 0, 0)).sdf_;
		v2 = map.FindVoxel(pos + make_int3(1, -1, 0)).sdf_;
		v3 = map.FindVoxel(pos + make_int3(1, 0, -1)).sdf_;
		if (isnan(v1) || isnan(v2) || isnan(v3))
			return false;
		n[1] = make_float3(v1 - sdf[0], sdf[2] - v2, sdf[5] - v3);

		v1 = map.FindVoxel(pos + make_int3(2, 1, 0)).sdf_;
		v2 = map.FindVoxel(pos + make_int3(1, 2, 0)).sdf_;
		v3 = map.FindVoxel(pos + make_int3(1, 1, -1)).sdf_;
		if (isnan(v1) || isnan(v2) || isnan(v3))
			return false;
		n[2] = make_float3(v1 - sdf[3], v2 - sdf[1], sdf[6] - v3);

		v1 = map.FindVoxel(pos + make_int3(-1, 1, 0)).sdf_;
		v2 = map.FindVoxel(pos + make_int3(0, 2, 0)).sdf_;
		v3 = map.FindVoxel(pos + make_int3(0, 1, -1)).sdf_;
		if (isnan(v1) || isnan(v2) || isnan(v3))
			return false;
		n[3] = make_float3(sdf[2] - v1, v2 - sdf[0], sdf[7] - v3);

		v1 = map.FindVoxel(pos + make_int3(-1, 0, 1)).sdf_;
		v2 = map.FindVoxel(pos + make_int3(0, -1, 1)).sdf_;
		v3 = map.FindVoxel(pos + make_int3(0, 0, 2)).sdf_;
		if (isnan(v1) || isnan(v2) || isnan(v3))
			return false;
		n[4] = make_float3(sdf[5] - v1, sdf[7] - v2, v3 - sdf[0]);

		v1 = map.FindVoxel(pos + make_int3(2, 0, 1)).sdf_;
		v2 = map.FindVoxel(pos + make_int3(1, -1, 1)).sdf_;
		v3 = map.FindVoxel(pos + make_int3(1, 0, 2)).sdf_;
		if (isnan(v1) || isnan(v2) || isnan(v3))
			return false;
		n[5] = make_float3(v1 - sdf[4], sdf[6] - v2, v3 - sdf[1]);

		v1 = map.FindVoxel(pos + make_int3(2, 1, 1)).sdf_;
		v2 = map.FindVoxel(pos + make_int3(1, 2, 1)).sdf_;
		v3 = map.FindVoxel(pos + make_int3(1, 1, 2)).sdf_;
		if (isnan(v1) || isnan(v2) || isnan(v3))
			return false;
		n[6] = make_float3(v1 - sdf[7], v2 - sdf[5], v3 - sdf[2]);

		v1 = map.FindVoxel(pos + make_int3(-1, 1, 1)).sdf_;
		v2 = map.FindVoxel(pos + make_int3(0, 2, 1)).sdf_;
		v3 = map.FindVoxel(pos + make_int3(0, 1, 2)).sdf_;
		if (isnan(v1) || isnan(v2) || isnan(v3))
			return false;
	...
	__device__ inline void MarchingCube()
	{
		...
		if (*noTriangles >= param.maxNumMeshTriangles || x >= *noBlocks)
			return;
		...
		int3 pos = blockPos[x] * BLOCK_SIZE;
		for (int i = 0; i < BLOCK_SIZE; ++i)
		{
			...
			{
				int tid = offset + i;
				if (tid >= param.maxNumMeshTriangles)
					return;

				vertices[tid * 3 + 0] = vlist[triangleTable.ptr(cubeIdx)[i * 3 + 0]] * param.voxel_size_;
				vertices[tid * 3 + 1] = vlist[triangleTable.ptr(cubeIdx)[i * 3 + 1]] * param.voxel_size_;
				vertices[tid * 3 + 2] = vlist[triangleTable.ptr(cubeIdx)[i * 3 + 2]] * param.voxel_size_;
				...
				DeviceArray<int3> & blockPoses)
{
	...
	dim3 thread(1024);
	dim3 block = dim3(div_up(state.maxNumHashEntries, thread.x));
	...
	thread = dim3(8, 8, 1);
	block = dim3(div_up(host_data, 16), 16, 1);
	...
	host_data = min(host_data, state.maxNumMeshTriangles);
